...
- (void)viewDidLoad {
    ...
    if (self.scanType != FrontOfIDCardScan) {
...
- (void)IDCardRecognit:(CVImageBufferRef)imageBuffer {
    ...
    if (CVPixelBufferLockBaseAddress(imageBuffer, 0) == kCVReturnSuccess) {
        ...
        if (ret <= 0) {
            ...
            IDCardModel *iDInfo = [[IDCardModel alloc] init];
            ...
            while(i < ret){
                ...
                if(xlen) {
                    ...
                    NSLog(@"ctype = %c, content = %@", ctype, [NSString stringWithCString:(char *)content encoding:gbkEncoding]);
                    if(ctype == 0x21) {
                        iDInfo.IDNumber = [NSString stringWithCString:(char *)content encoding:gbkEncoding];
                    } else if(ctype == 0x22) {
                        ...
                        iDInfo.authority = [NSString stringWithCString:(char *)content encoding:gbkEncoding];
                    } else if(ctype == 0x27) {
                        iDInfo.validity = [NSString stringWithCString:(char *)content encoding:gbkEncoding];
                    }
                }
            }
            
            if (iDInfo) {// 读取到身份证信息，实例化出IDInfo对象后，截取身份证的有效区域，获取到图像
                NSLog(@"\n正面\n姓名：%@\n性别：%@\n民族：%@\n住址：%@\n公民身份证号码：%@\n\n反面\n签发机关：%@\n有效期限：%@",iDInfo.name,iDInfo.gender,iDInfo.nation,iDInfo.address,iDInfo.IDNumber,iDInfo.authority,iDInfo.validity);
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    self.IDCardInfoBlock(iDInfo, subImage);
